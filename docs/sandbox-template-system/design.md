...
LiteBoxd 系统采用**强制模版化**的设计理念：所有沙箱必须通过模版创建，不允许直接配置创建。这种设计确保了沙箱配置的一致性、可追溯性和可维护性。参考 [E2B](https://e2b.dev/docs/sandbox-template) 的设计理念，设计了一套轻量级的沙箱模版系统。

### 1.2 设计目标

- **强制模版化**: 所有沙箱必须通过模版创建，确保配置来源可追溯
- **简单易用**: 通过 YAML 文件定义模版，无需额外的构建步骤
- **快速实例化**: 从模版创建沙箱只需指定模版 ID
- **灵活覆盖**: 支持通过 overrides 参数覆盖部分配置
- **版本管理**: 支持模版的版本控制和回滚
...
{
  "template": "python-data-science",  # 必填：模版名称
...
│ 4. 创建 Pod                                                               │
│    - 使用合并后的配置创建 K8s Pod                                           │
│    - 添加模版注解 (template, template-version)                              │
...
1. **一致性**: 所有沙箱都有明确的配置来源，便于审计和管理
2. **可追溯**: 每个沙箱都记录了使用的模版和版本
3. **可维护**: 模版集中管理，配置变更方便
4. **安全性**: 防止用户使用未经测试的镜像配置
5. **资源管理**: 配合镜像预拉取功能，加速沙箱启动
6. **灵活性**: 通过 overrides 参数支持一定程度的配置覆盖
